...
O site (`docs/`) tem o próprio `Gemfile` e que fica em `docs/`. Você pode executar os comandos `bundle` diretamente neste diretório ou utilizar o script `serve.sh`, que executa o seguinte:

1. Aponta o Bundler para `docs/content/Gemfile` (via `BUNDLE_GEMFILE`), não
