---
layout: default
title: Fundamentos (Containers)
---

# 1. Fundamentos

A base técnica indispensável: sistemas operacionais, redes, automação e contêineres.

## Containers

Contêiner é uma forma de isolar um ambiente para executar códigos, aplicações e assim por diante.
Docker é a ferramenta mais popular para criação e execução de contêineres.

**Trilha de Estudo:**

* <i class="fas fa-video"></i> **Vídeo introdutório:** [[Mayk Brito] Como funciona o Docker? (explicação SIMPLES)](https://youtu.be/IY-ceuwqnns)
* <i class="fas fa-graduation-cap"></i> **Treinamento:** [[LinuxTips] Docker Essentials](https://linuxtips.io/treinamento/docker-essentials/)

**Laboratório Girus:** acesse o seu ambiente [Girus]({{ '/content/modules/introducao/preparacao-ambiente/' | relative_url }}), vá em Laboratórios e filtre por *Docker*. Faça os seguintes treinamentos:

* Introdução ao Docker
* Gerenciamento de Containers Docker
* Redes no Docker: Conceitos e Implementação
* Persistência de Dados com Docker Volumes
* Introdução ao Docker Compose

Pratique cada um desses laboratórios antes de seguir para o próximo tópico.

{% include next-steps.html
   prev_url="/content/modules/fundamentos/git/"
   prev_title="Git"
   next_url="/content/modules/fundamentos/exercise/"
   next_title="Exercícios (Fundamentos)"
%}
